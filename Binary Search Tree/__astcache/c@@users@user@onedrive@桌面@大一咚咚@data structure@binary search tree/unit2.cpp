...
struct BSTreeNode *InsertBST(struct BSTreeNode *node, int x)    //遞迴作法
{
	if(node==NULL)
		return NewBSTNode(x);
	if(x<node->data)
...
int InsertBSTNR(int x)          //非遞迴作法
{
	struct BSTreeNode *p, *q;
	p = root; q = NULL;   //p是存放x的新節點 q是來記住p的父親節點的位置
	while (p != NULL)
	{
		q = p;
